...
cd "C:\Users\sg8874\Documents\Advanced Causal Inference Project"  // Change directory to your folder
local files: dir . files "*.csv"  // Get all CSV files in the folder

foreach file in `files' {
    local name = subinstr("`file'", ".csv", "", .)  // Remove .csv from filename
    import delimited "`file'", clear
    save "`name'.dta", replace
}


local files: dir . files "*.dta"  // Get all .dta files in the folder

local first = 1  // Flag to check if it's the first file

foreach file in `files' {
    use "`file'", clear  // Load the current file
    
    // Manually extract year assuming the filename contains a 4-digit year
    local year = substr("`file'", -8, 4)  // Extracts year from the last 8 characters of the filename

    gen year = real("`year'")  // Create a numeric 'year' variable

    if `first' == 1 {
        save "combined_data.dta", replace  // Save the first dataset
        local first = 0  // Set flag to indicate the first file is processed
    }
    else {
        append using "combined_data.dta"  // Append subsequent datasets
        save "combined_data.dta", replace  // Save the combined dataset
    }
}

display "Appending complete with new variable 'year'. File saved as combined_data.dta"
...
* Recode admcon7 into a binary variable indicating test-optional status
gen test_optional = .
replace test_optional = 1 if admcon7 == 2 | admcon7 == 3
replace test_optional = 0 if admcon7 == 1

drop if inlist(admcon7, -1, -2, 2, 4, 5)

* Set the panel data structure
xtset unitid year

* Create the lag of test_optional status
gen prev_test_optional = L.test_optional

* Create a variable to track if the status changed
gen status_changed = (test_optional != prev_test_optional) & !missing(prev_test_optional)

save "combined_data.dta", replace

//use "C:\Users\sg8874\Downloads\combined_data.dta", replace

//preserve  // Save current dataset

//twoway line test_optional year, title("Proportion of Test-Optional Universities Over Time") xlabel(2005(1)2018) ylabel(0(0.1)1) ytitle("Proportion of Test-Optional") xtitle("Year") lwidth(medium) lcolor(blue)

//restore  // Get your original dataset back (before collapsing)


//preserve  // Save current dataset

* Step 1: Count how many changed each year
//collapse (sum) status_changed, by(year)
* Step 2: Plot it
//twoway (line status_changed year), title("Universities Changing Test-Optional Status Over Time") xlabel(2005(1)2018) ylabel(, angle(0)) ytitle("Number of Universities") xtitle("Year") 

//restore  // Get your original dataset back (before collapsing)

//save "C:\Users\sg8874\Downloads\combined_data.dta", replace


cd "C:\Users\sg8874\Downloads\Retention Rates Data"

clear all
local files: dir . files "ef*.dta"  // Assuming files start with 'ef'
...
foreach file in `files' {
    ...
    // Calculate position to extract year
    local len = strlen("`file'")
    local yearstr = substr("`file'", `=`len'-7', 4)
    gen year = real("`yearstr'")  // Create numeric year variable

    if `first' == 1 {
        save "COMBINED_RETENTION_RATES_DATA.dta", replace
        local first = 0
    }
    else {
        append using "COMBINED_RETENTION_RATES_DATA.dta"
        save "COMBINED_RETENTION_RATES_DATA.dta", replace
    }
}

order year, first
display "Appending complete with correct 'year' values!"
...
use "C:\Users\sg8874\Downloads\Retention Rates Data\COMBINED_RETENTION_RATES_DATA.dta", replace
sort unitid year
save "C:\Users\sg8874\Downloads\Retention Rates Data\COMBINED_RETENTION_RATES_DATA.dta", replace

use "C:\Users\sg8874\Downloads\combined_data.dta", clear
merge 1:1 unitid year using "C:\Users\sg8874\Downloads\Retention Rates Data\COMBINED_RETENTION_RATES_DATA.dta"

keep if _merge == 3
drop _merge

save "MERGED_RETENTION_RATES_COMBINED_DATA.dta", replace

use "MERGED_RETENTION_RATES_COMBINED_DATA.dta", replace
...
label variable xugenter "Imputation field for ugentern - Total entering students at the undergraduate level, fall 2018"
label variable ugentern "Total entering students at the undergraduate level, fall 2018"
...
label variable xrrftct "Imputation field for rrftct - Full-time fall 2017 cohort"
label variable rrftct   "Full-time fall 2017 cohort"
label variable xrrftex "Imputation field for rrftex - Exclusions from full-time fall 2017 cohort"
label variable rrftex   "Exclusions from full-time fall 2017 cohort"
label variable xrrftin "Imputation field for rrftin - Inclusions to the full-time fall 2017 cohort"
label variable rrftin   "Inclusions to the full-time fall 2017 cohort"
label variable xrrftcta "Imputation field for rrftcta - Full-time adjusted fall 2017 cohort"
label variable rrftcta  "Full-time adjusted fall 2017 cohort"
label variable xret_nmf "Imputation field for ret_nmf - Students from the full-time adjusted fall 2017 cohort enrolled in fall 2018"
label variable ret_nmf  "Students from the full-time adjusted fall 2017 cohort enrolled in fall 2018"
label variable xret_pcf "Imputation field for ret_pcf - Full-time retention rate, 2018"
label variable ret_pcf  "Full-time retention rate, 2018"
label variable xrrptct "Imputation field for rrptct - Part-time fall 2017 cohort"
label variable rrptct   "Part-time fall 2017 cohort"
label variable xrrptex "Imputation field for rrptex - Exclusions from part-time fall 2017 cohort"
label variable rrptex   "Exclusions from part-time fall 2017 cohort"
label variable xrrptin "Imputation field for rrptin - Inclusions to the part-time fall 2017 cohort"
label variable rrptin   "Inclusions to the part-time fall 2017 cohort"
label variable xrrptcta "Imputation field for rrptcta - Part-time adjusted fall 2017 cohort"
label variable rrptcta  "Part-time adjusted fall 2017 cohort"
label variable xret_nmp "Imputation field for ret_nmp - Students from the part-time adjusted fall 2017 cohort enrolled in fall 2018"
label variable ret_nmp  "Students from the part-time adjusted fall 2017 cohort enrolled in fall 2018"
label variable xret_pcp "Imputation field for ret_pcp - Part-time retention rate, 2018"
label variable ret_pcp  "Part-time retention rate, 2018"
...
* Generate treatment year (if not already present)
gen treat_year = .
replace treat_year = year if test_optional == 1 & missing(treat_year)

* Carry forward first year of treatment for each treated unit
bysort unitid (year): replace treat_year = treat_year[_n-1] if missing(treat_year)

* Generate relative time indicators (event-time dummies)
gen rel_year = year - treat_year

* Optional: exclude year 0 (year of treatment) or make it the reference
* e.g., exclude rel_year == 0 as base period
forvalues k = -5/5 {
    gen event_`k' = 0
    replace event_`k' = 1 if rel_year == `k' & test_optional == 1
}

* Now estimate the model (excluding event_0 as base)
xtset unitid year

xtreg retention_rate ///
    event_* ///
    i.year ///
    institutional_controls*, fe cluster(unitid)
...
*----------------------------------------------------*
* Data Cleaning Script for Test-Optional Retention Study
*----------------------------------------------------*

* Load the original merged dataset
use "MERGED_RETENTION_RATES_COMBINED_DATA.dta", clear

* Keep only relevant variables for DiD and PSM
keep unitid year test_optional status_changed ret_pcf ///
     cntlaffi pubprime alloncam roomamt boardamt rmbrdamt ///
     applcn admssn enrlft satvr25 satvr75 satmt25 satmt75 ///
     actcm25 actcm75 actmt25 actmt75 disabpct stufacr libfac ///
     tuitpl tuitpl1 tuitpl2 tuitpl3 tuitpl4

* Drop rows with missing outcome or treatment indicator
drop if missing(unitid) | missing(year) | missing(test_optional) | missing(ret_pcf)

* Save the cleaned dataset
save " cleaned_retention_dataset.dta", replace
...
use "cleaned_retention_dataset.dta", clear

*---------------------------------------------*
* Encode categorical variables if needed
*---------------------------------------------*
encode cntlaffi, gen(ctrl_affil)
encode pubprime, gen(public_status)

*---------------------------------------------*
* Basic Summary Statistics (Optional)
*---------------------------------------------*
summarize ret_pcf applcn admssn enrlft satvr25 satvr75 satmt25 satmt75 actcm25 actcm75 actmt25 actmt75 disabpct stufacr tuitpl tuitpl1 tuitpl2 tuitpl3 tuitpl4

*---------------------------------------------*
* Setting up for csdid
*---------------------------------------------*

* Create treatment variable
gen treated = test_optional

* Group variable: first year when treated
gen gvar = .
bysort unitid (year): replace gvar = year if treated == 1 & missing(gvar)
...
csdid ret_pcf ///
    ...
    ivar(unitid) ///
    method(dripw) ///
    covariates(applcn admssn enrlft satvr25 satvr75 satmt25 satmt75 ///
               actcm25 actcm75 actmt25 actmt75 disabpct ///
               stufacr roomamt boardamt rmbrdamt tuitpl tuitpl1 tuitpl2 tuitpl3 tuitpl4)
...
*---------------------------------------------*
* (Optional) Aggregate ATT across groups
*---------------------------------------------*
estat agg
